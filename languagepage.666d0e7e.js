...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    (0, _auth.onAuthStateChanged)((0, _firebaseJs.auth), async (user)=>{
        ...
        } else // Якщо локально сесії немає взагалі — тоді точно кидаємо на index.html
        // Якщо ж сесія колись була, даємо трохи більше часу на відновлення
        if (!hasLocalSession) setTimeout(()=>{
            if (!(0, _firebaseJs.auth).currentUser) window.location.href = './index.html';
        }, 1000);
    });
    // Запобіжник: якщо за 2.5 секунди стейт не змінився і користувача немає
    setTimeout(async ()=>{
        if (!isAuthorized && !(0, _firebaseJs.auth).currentUser) // Остання перевірка: можливо Firebase просто довго думає, 
...
function updateNavigationLinks(langCode) {
    ...
    if (glossaryLink) glossaryLink.href = `./glossary.html?lang=${langCode}`;
    const rulesLink = document.querySelector('.web-navigation__page-link[href*="rules.html"]');
    if (rulesLink) rulesLink.href = `./rules.html?lang=${rulesLink.getAttribute('href')?.includes('lang=') ? '' : 'lang=' + langCode}`; // безпечне оновлення
}
function initHeaderEventListeners() {
    ...
    document.addEventListener('click', (e)=>{
        if (userInfoPopup && !userInfoPopup.classList.contains('disable')) {
            if (!userInfoPopup.contains(e.target) && !userMenuTrigger?.contains(e.target)) userInfoPopup.classList.add('disable');
        }
    });
}
...
},{"./firebase.js":"8uCPj","firebase/auth":"4ZBbi","firebase/firestore":"3RBs1"}]},["lu94s","8WFLb"], "8WFLb", "parcelRequiree231", {})
